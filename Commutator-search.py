...
class MonomialCommutator:
    ...
    def generateCommutator(self) -> np.ndarray:
        ...
        commutator_component = np.ones((N,M))
        return commutator_component, commutator_component

    def x_derivative(self,polynomial:np.ndarray):
        derivative_x = np.zeros(polynomial.shape)
        for i in range(1,polynomial.shape[0]):
            derivative_x[i-1] = polynomial[i] * i

        return derivative_x

    def y_derivative(self,polynomial:np.ndarray):
        derivative_y = np.zeros(polynomial.shape)
        for i in range(1,polynomial.shape[1]):
            derivative_y[:,i-1] = polynomial[:,i] * i
    ...
    def create_BIG_matrix(self, polynomials: list[np.ndarray],
                          monomial_derivatives: list[Monomial],
                          polynomial_derivatives: list[np.ndarray],
                          ) -> np.ndarray:
        P = polynomials[0]
        Q = polynomials[1]
        ...
        D_P_1 = self.monomials[0].coeff * polynomial_derivatives[0]
        D_P_2 = self.monomials[1].coeff * polynomial_derivatives[1]

        k3 = self.monomials[0].powers[0]
        m3 = self.monomials[0].powers[1]
        end31 = k3 + polynomial_derivatives[0].shape[0]
        end32 = m3 + polynomial_derivatives[0].shape[1]
        BIG_Matrix[2, k3:end31, m3:end32] = D_P_1*(-1)

        k4 = self.monomials[1].powers[0]
        m4 = self.monomials[1].powers[1]
        end41 = k4 + polynomial_derivatives[1].shape[0]
        end42 = m4 + polynomial_derivatives[1].shape[1]
